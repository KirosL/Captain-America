...
class Aliados extends Component {
    ...
    peticionGet = () => {
        fireDb.child("Aliados").on("value", (aliado) => {
        if (aliado.val() !== null) {
    ...
    peticionPost=()=>{
        fireDb.child("Aliados").push(this.state.form,
        error=>{
    ...
    peticionPut=()=>{
        fireDb.child(`Aliados/${this.state.id}`).set(
        this.state.form,
        error=>{
            if(error)console.log(error)
        });
    ...
    peticionDelete=()=>{
        ...
        {
        fireDb.child(`Aliados/${this.state.id}`).remove(
        error=>{
            if(error)console.log(error)
        });
        }
